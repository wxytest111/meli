...
//  CCMy
//
//  Created by baby on 14-5-27.
//  Copyright (c) 2014年 MobiMirage. All rights reserved.
//

#import "CCMyKOpenUDID.h"
...
static NSString * CCMyOpenUDIDSessionCache = nil;
//static NSString * const CCMyOpenUDIDDescription = @"OpenUDID_with_iOS6_Support";
static NSString * const CCMyOpenUDIDKey = @"OpenUDID";
static NSString * const CCMyOpenUDIDSlotKey = @"OpenUDID_slot";
static NSString * const CCMyOpenUDIDAppUIDKey = @"OpenUDID_appUID";
static NSString * const CCMyOpenUDIDTSKey = @"OpenUDID_createdTS";
static NSString * const CCMyOpenUDIDOOTSKey = @"OpenUDID_optOutTS";
static NSString * const CCMyOpenUDIDDomain = @"org.OpenUDID";
static NSString * const CCMyOpenUDIDSlotPBPrefix = @"org.OpenUDID.slot.";
static int const CCMyOpenUDIDRedundancySlots = 100;

@interface CCMyOpenUDID (Private)
...
+ (void) _setDict:(id)dict forPasteboard:(id)pboard {
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR
    [pboard setData:[NSKeyedArchiver archivedDataWithRootObject:dict] forPasteboardType:CCMyOpenUDIDDomain];
#else
    [pboard setData:[NSKeyedArchiver archivedDataWithRootObject:dict] forType:CCMyOpenUDIDDomain];
#endif
}
...
+ (NSMutableDictionary*) _getDictFromPasteboard:(id)pboard {
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR
    id item = [pboard dataForPasteboardType:CCMyOpenUDIDDomain];
#else
    id item = [pboard dataForType:CCMyOpenUDIDDomain];
#endif
    if (item) {
        @try{
...
//
+ (NSString*) value {
    return [CCMyOpenUDID valueWithError:nil];
}

+ (NSString*) valueWithError:(NSError **)error {
    
    if (CCMyOpenUDIDSessionCache!=nil) {
        if (error!=nil)
            *error = [NSError errorWithDomain:CCMyOpenUDIDDomain
        ...
        return CCMyOpenUDIDSessionCache;
    }
    ...
    //
    NSString * appUID = [defaults objectForKey:CCMyOpenUDIDAppUIDKey];
    if(appUID == nil)
    {
    ...
    //
    id localDict = [defaults objectForKey:CCMyOpenUDIDKey];
    if ([localDict isKindOfClass:[NSDictionary class]]) {
        ...
        openUDID = [localDict objectForKey:CCMyOpenUDIDKey];
        myRedundancySlotPBid = [localDict objectForKey:CCMyOpenUDIDSlotKey];
        optedOutDate = [localDict objectForKey:CCMyOpenUDIDOOTSKey];
    ...
    NSMutableDictionary* frequencyDict = [NSMutableDictionary dictionaryWithCapacity:CCMyOpenUDIDRedundancySlots];
    for (int n=0; n<CCMyOpenUDIDRedundancySlots; n++) {
        NSString* slotPBid = [NSString stringWithFormat:@"%@%d",CCMyOpenUDIDSlotPBPrefix,n];
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR
        ...
        if (slotPB==nil) {
            ...
            NSDictionary* dict = [CCMyOpenUDID _getDictFromPasteboard:slotPB];
            NSString* oudid = [dict objectForKey:CCMyOpenUDIDKey];
            ...
            NSString* gid = [dict objectForKey:CCMyOpenUDIDAppUIDKey];
            if (gid!=nil && [gid isEqualToString:appUID]) {
                ...
                if (optedOut) {
                    optedOutDate = [dict objectForKey:CCMyOpenUDIDOOTSKey];
                    optedOut = optedOutDate!=nil;
                }
            }
        }
    ...
    if (openUDID==nil) {
        if (mostReliableOpenUDID==nil) {
            ...
            //
            openUDID = [CCMyOpenUDID _generateFreshOpenUDID];
        } else {
        ...
        if (localDict==nil) {
            ...
            [localDict setObject:openUDID forKey:CCMyOpenUDIDKey];
            [localDict setObject:appUID forKey:CCMyOpenUDIDAppUIDKey];
            [localDict setObject:[NSDate date] forKey:CCMyOpenUDIDTSKey];
            if (optedOut) [localDict setObject:optedOutDate forKey:CCMyOpenUDIDTSKey];
    ...
    if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        //
        if (localDict) {
            [localDict setObject:availableSlotPBid forKey:CCMyOpenUDIDSlotKey];
            saveLocalDictToDefaults = YES;
        }
        ...
        if (openUDID && localDict)
            [CCMyOpenUDID _setDict:localDict forPasteboard:slotPB];
    }
    
    // Save the dictionary locally if applicable
    //
    if (localDict && saveLocalDictToDefaults)
        [defaults setObject:localDict forKey:CCMyOpenUDIDKey];
    ...
    //
    if (optedOut) {
        if (error!=nil) *error = [NSError errorWithDomain:CCMyOpenUDIDDomain
        ...
        CCMyOpenUDIDSessionCache = [[NSString stringWithFormat:@"%040x",0] retain];
        return CCMyOpenUDIDSessionCache;
    }
    ...
    CCMyOpenUDIDSessionCache = [openUDID retain];
    return CCMyOpenUDIDSessionCache;
}
...
+ (void) setOptOut:(BOOL)optOutValue {
    ...
    id dict = [defaults objectForKey:CCMyOpenUDIDKey];
    ...
    if (optOutValue)
        [dict setObject:[NSDate date] forKey:CCMyOpenUDIDOOTSKey];
    else
        [dict removeObjectForKey:CCMyOpenUDIDOOTSKey];
    
    // store the dictionary locally
    [defaults setObject:dict forKey:CCMyOpenUDIDKey];
    
    OpenUDIDLog(@"Local dict after opt-out = %@",dict);
    
    // reset memory cache
    CCMyOpenUDIDSessionCache = nil;
