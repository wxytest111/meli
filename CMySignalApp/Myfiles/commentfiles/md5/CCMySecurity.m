...
//  SDKDEMO
//
//  Created by baby on 14-6-6.
//  Copyright (c) 2014年 Seven. All rights reserved.
//

#import "CCMySecurity.h"
#import "NSData+Base64.h"


@implementation CCMySecurity


-(void) showcrypt
{
    NSString *sfilename = [[NSBundle mainBundle] pathForResource:@"pkcs" ofType:@"p12"];
    if (sfilename == nil)
    {
        NSLog(@"Can not find pkcs.p12");
        return ;
    }
    NSString* spassword = @"1234qwer";
    OSStatus status1 = [ self extractEveryThingFromPKCS12File:sfilename passphrase:spassword ];
    if (status1 == noErr)
    {
        ;
    }
    return ;
    /*
    NSString* astring = [ NSString stringWithFormat:@"stringwith%@",@"teststring" ];
    NSLog(@"astring:%@", astring);
    NSData* pdata1 = [ astring dataUsingEncoding:NSUTF8StringEncoding ];
    NSData* pdata2 = [ self encryptWithPublicKey:pdata1 ];
    NSData* pdata3 = [ self decryptWithPrivateKey:pdata2 ];
    NSString* str1 = [ [NSString alloc] initWithData:pdata3 encoding:NSUTF8StringEncoding ];
    NSLog(@"str1:%@", str1);
    */
}
...
- (NSData *)encryptWithPublicKey:(NSData *)plainData
{
    ...
    for (int i = 0; i < blockCount; i++)
    {
        ...
        OSStatus status = SecKeyEncrypt(_publicKey,
                                        kSecPaddingPKCS1,
...
- (NSData *)decryptWithPrivateKey:(NSData *)cipherData
{
    ...
    for (int i = 0; i < blockCount; i++)
    {
        ...
        OSStatus status = SecKeyDecrypt(_privateKey,
                                        kSecPaddingPKCS1,
                                        (const uint8_t *)[dataSegment bytes],
                                        dataSegmentRealSize,
                                        plainBuffer,
