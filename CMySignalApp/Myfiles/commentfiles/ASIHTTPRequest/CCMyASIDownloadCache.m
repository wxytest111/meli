...
#import "CCMyASIDownloadCache.h"
#import "CCMyASIHTTPRequest.h"
#import <CommonCrypto/CommonHMAC.h>

static CCMyASIDownloadCache *sharedCache = nil;

static NSString *CCMysessionCacheFolder = @"SessionStore";
static NSString *CCMypermanentCacheFolder = @"PermanentStore";
static NSArray *CCMyfileExtensionsToHandleAsHTML = nil;

@interface CCMyASIDownloadCache ()
+ (NSString *)keyForURL:(NSURL *)url;
- (NSString *)pathToFile:(NSString *)file;
@end

@implementation CCMyASIDownloadCache

+ (void)initialize
{
	if (self == [CCMyASIDownloadCache class]) {
		...
		CCMyfileExtensionsToHandleAsHTML = [[NSArray alloc] initWithObjects:@"asp",@"aspx",@"jsp",@"php",@"rb",@"py",@"pl",@"cgi", nil];
	}
}
...
- (void)setStoragePath:(NSString *)path
{
	...
	BOOL isDirectory = NO;
	NSArray *directories = [NSArray arrayWithObjects:path,[path stringByAppendingPathComponent:CCMysessionCacheFolder],[path stringByAppendingPathComponent:CCMypermanentCacheFolder],nil];
...
- (void)updateExpiryForRequest:(CCMyASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
...
{
  return [CCMyASIHTTPRequest expiryDateForRequest:request maxAge:maxAge];
}

- (void)storeResponseForRequest:(CCMyASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	[[self accessLock] lock];
...
{
	return CCMyfileExtensionsToHandleAsHTML;
}
...
- (NSString *)pathToFile:(NSString *)file
{
	...
	NSString *dataPath = [[[self storagePath] stringByAppendingPathComponent:CCMysessionCacheFolder] stringByAppendingPathComponent:file];
	if ([fileManager fileExistsAtPath:dataPath]) {
		[[self accessLock] unlock];
		return dataPath;
	}
	// Look in the permanent store
	dataPath = [[[self storagePath] stringByAppendingPathComponent:CCMypermanentCacheFolder] stringByAppendingPathComponent:file];
	if ([fileManager fileExistsAtPath:dataPath]) {
		[[self accessLock] unlock];
		return dataPath;
	}
...
- (NSString *)pathToStoreCachedResponseDataForRequest:(CCMyASIHTTPRequest *)request
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
	...
	NSString *path = [[self storagePath] stringByAppendingPathComponent:([request cacheStoragePolicy] == ASICacheForSessionDurationCacheStoragePolicy ? CCMysessionCacheFolder : CCMypermanentCacheFolder)];
...
- (NSString *)pathToStoreCachedResponseHeadersForRequest:(CCMyASIHTTPRequest *)request
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
...
- (void)removeCachedDataForRequest:(CCMyASIHTTPRequest *)request
{
	[self removeCachedDataForURL:[request url]];
}

- (BOOL)isCachedDataCurrentForRequest:(CCMyASIHTTPRequest *)request
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
...
- (CCMyASICachePolicy)defaultCachePolicy
{
	[[self accessLock] lock];
	CCMyASICachePolicy cp = defaultCachePolicy;
	[[self accessLock] unlock];
	return cp;
}


- (void)setDefaultCachePolicy:(CCMyASICachePolicy)cachePolicy
{
	[[self accessLock] lock];
	if (!cachePolicy) {
...
- (void)clearCachedResponsesForStoragePolicy:(CCMyASICacheStoragePolicy)storagePolicy
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
	...
	NSString *path = [[self storagePath] stringByAppendingPathComponent:(storagePolicy == ASICacheForSessionDurationCacheStoragePolicy ? CCMysessionCacheFolder : CCMypermanentCacheFolder)];
...
+ (BOOL)serverAllowsResponseCachingForRequest:(CCMyASIHTTPRequest *)request
{
...
- (BOOL)canUseCachedDataForRequest:(CCMyASIHTTPRequest *)request
{
