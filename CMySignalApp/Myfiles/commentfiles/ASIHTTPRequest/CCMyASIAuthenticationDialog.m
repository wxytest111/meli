...
static CCMyASIAuthenticationDialog *sharedDialog = nil;
BOOL CCMyisDismissing = NO;
static NSMutableArray *CCMyrequestsNeedingAuthentication = nil;

static const NSUInteger CCMykUsernameRow = 0;
static const NSUInteger CCMykUsernameSection = 0;
static const NSUInteger CCMykPasswordRow = 1;
static const NSUInteger CCMykPasswordSection = 0;
static const NSUInteger CCMykDomainRow = 0;
static const NSUInteger CCMykDomainSection = 1;


@implementation CCMyASIAutorotatingViewController
...
@interface CCMyASIAuthenticationDialog ()
- (void)showTitle;
- (void)show;
...
@implementation CCMyASIAuthenticationDialog

#pragma mark init / dealloc

+ (void)initialize
{
	if (self == [CCMyASIAuthenticationDialog class]) {
		CCMyrequestsNeedingAuthentication = [[NSMutableArray array] retain];
	}
}

+ (void)presentAuthenticationDialogForRequest:(CCMyASIHTTPRequest *)theRequest
{
	...
	if (!sharedDialog) {
		...
		[CCMyrequestsNeedingAuthentication addObject:theRequest];
	}
}
...
- (UIViewController *)presentingController
{
	if (!presentingController) {
		presentingController = [[CCMyASIAutorotatingViewController alloc] initWithNibName:nil bundle:nil];
...
{
	return [self textFieldInRow:CCMykUsernameRow section:CCMykUsernameSection];
}

- (UITextField *)passwordField
{
	return [self textFieldInRow:CCMykPasswordRow section:CCMykPasswordSection];
}

- (UITextField *)domainField
{
	return [self textFieldInRow:CCMykDomainRow section:CCMykDomainSection];
}
...
- (void)cancelAuthenticationFromDialog:(id)sender
{
	for (CCMyASIHTTPRequest *theRequest in [self requestsRequiringTheseCredentials]) {
		[theRequest cancelAuthentication];
		[CCMyrequestsNeedingAuthentication removeObject:theRequest];
	}
	[self dismiss];
}
...
- (NSArray *)requestsRequiringTheseCredentials
{
	...
	for (CCMyASIHTTPRequest *otherRequest in CCMyrequestsNeedingAuthentication) {
...
- (void)presentNextDialog
{
	if ([CCMyrequestsNeedingAuthentication count]) {
		CCMyASIHTTPRequest *nextRequest = [CCMyrequestsNeedingAuthentication objectAtIndex:0];
		[CCMyrequestsNeedingAuthentication removeObjectAtIndex:0];
