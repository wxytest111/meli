...
#if TARGET_OS_IPHONE
#import "CCMyReachability.h"
#import "CCMyASIAuthenticationDialog.h"
#import <MobileCoreServices/MobileCoreServices.h>
#else
#import <SystemConfiguration/SystemConfiguration.h>
#endif
#import "CCMyASIInputStream.h"
#import "CCMyASIDataDecompressor.h"
#import "CCMyASIDataCompressor.h"

// Automatically set on build
NSString *CCMyASIHTTPRequestVersion = @"v1.8.1-61 2011-09-19";

static NSString *CCMydefaultUserAgent = nil;

NSString* const CCMyNetworkRequestErrorDomain = @"ASIHTTPRequestErrorDomain";

static NSString *CCMyASIHTTPRequestRunLoopMode = @"ASIHTTPRequestRunLoopMode";

static const CFOptionFlags CCMykNetworkEvents =  kCFStreamEventHasBytesAvailable | kCFStreamEventEndEncountered | kCFStreamEventErrorOccurred;

// In memory caches of credentials, used on when useSessionPersistence is YES
static NSMutableArray *CCMysessionCredentialsStore = nil;
static NSMutableArray *CCMysessionProxyCredentialsStore = nil;

// This lock mediates access to session credentials
static NSRecursiveLock *CCMysessionCredentialsLock = nil;

// We keep track of cookies we have received here so we can remove them from the sharedHTTPCookieStorage later
static NSMutableArray *CCMysessionCookies = nil;

// The number of times we will allow requests to redirect before we fail with a redirection error
const int CCMyRedirectionLimit = 5;

// The default number of seconds to use for a timeout
static NSTimeInterval CCMydefaultTimeOutSeconds = 10;

static void CCMyReadStreamClientCallBack(CFReadStreamRef readStream, CFStreamEventType type, void *clientCallBackInfo) {
    [((CCMyASIHTTPRequest*)clientCallBackInfo) handleNetworkEvent: type];
}

// This lock prevents the operation from being cancelled while it is trying to update the progress, and vice versa
static NSRecursiveLock *CCMyprogressLock;

static NSError *CCMyASIRequestCancelledError;
static NSError *CCMyASIRequestTimedOutError;
static NSError *CCMyASIAuthenticationError;
static NSError *CCMyASIUnableToCreateRequestError;
static NSError *CCMyASITooMuchRedirectionError;

static NSMutableArray *CCMybandwidthUsageTracker = nil;
static unsigned long CCMyaverageBandwidthUsedPerSecond = 0;
...
static unsigned int CCMynextConnectionNumberToCreate = 0;
...
static NSMutableArray * CCMypersistentConnectionsPool = nil;

// Mediates access to the persistent connections pool
static NSRecursiveLock *CCMyconnectionsLock = nil;
...
static NSRecursiveLock *CCMydelegateAuthenticationLock = nil;
...
static NSDate *CCMythrottleWakeUpTime = nil;

static id <CCMyASICacheDelegate> CCMydefaultCache = nil;

// Used for tracking when requests are using the network
static unsigned int CCMyrunningRequestCount = 0;
...
static BOOL CCMyshouldUpdateNetworkActivityIndicator = YES;

// The thread all requests will run on
// Hangs around forever, but will be blocked unless there are requests underway
static NSThread *CCMynetworkThread = nil;

static NSOperationQueue *CCMysharedQueue = nil;

// Private stuff
@interface CCMyASIHTTPRequest ()
...
- (void)fetchPACFile;
- (void)finishedDownloadingPACFile:(CCMyASIHTTPRequest *)theRequest;
...
@property (assign) CCMyASIAuthenticationState authenticationNeeded;
...
@property (retain, nonatomic) CCMyASIHTTPRequest *PACFileRequest;
...
@implementation CCMyASIHTTPRequest

#pragma mark init / dealloc

+ (void)initialize
{
	if (self == [CCMyASIHTTPRequest class]) {
		CCMypersistentConnectionsPool = [[NSMutableArray alloc] init];
		CCMyconnectionsLock = [[NSRecursiveLock alloc] init];
		CCMyprogressLock = [[NSRecursiveLock alloc] init];
		CCMybandwidthThrottlingLock = [[NSLock alloc] init];
		CCMysessionCookiesLock = [[NSRecursiveLock alloc] init];
		CCMysessionCredentialsLock = [[NSRecursiveLock alloc] init];
		CCMydelegateAuthenticationLock = [[NSRecursiveLock alloc] init];
		CCMybandwidthUsageTracker = [[NSMutableArray alloc] initWithCapacity:5];
		CCMyASIRequestTimedOutError = [[NSError alloc] initWithDomain:CCMyNetworkRequestErrorDomain code:ASIRequestTimedOutErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request timed out",NSLocalizedDescriptionKey,nil]];
		CCMyASIAuthenticationError = [[NSError alloc] initWithDomain:CCMyNetworkRequestErrorDomain code:ASIAuthenticationErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Authentication needed",NSLocalizedDescriptionKey,nil]];
		CCMyASIRequestCancelledError = [[NSError alloc] initWithDomain:CCMyNetworkRequestErrorDomain code:ASIRequestCancelledErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request was cancelled",NSLocalizedDescriptionKey,nil]];
		CCMyASIUnableToCreateRequestError = [[NSError alloc] initWithDomain:CCMyNetworkRequestErrorDomain code:ASIUnableToCreateRequestErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to create request (bad url?)",NSLocalizedDescriptionKey,nil]];
		CCMyASITooMuchRedirectionError = [[NSError alloc] initWithDomain:CCMyNetworkRequestErrorDomain code:ASITooMuchRedirectionErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request failed because it redirected too many times",NSLocalizedDescriptionKey,nil]];
		CCMysharedQueue = [[NSOperationQueue alloc] init];
		[CCMysharedQueue setMaxConcurrentOperationCount:4];
...
- (id)initWithURL:(NSURL *)newURL
{
	...
	[self setTimeOutSeconds:[CCMyASIHTTPRequest defaultTimeOutSeconds]];
...
+ (id)requestWithURL:(NSURL *)newURL usingCache:(id <CCMyASICacheDelegate>)cache
{
	return [self requestWithURL:newURL usingCache:cache andCachePolicy:ASIUseDefaultCachePolicy];
}

+ (id)requestWithURL:(NSURL *)newURL usingCache:(id <CCMyASICacheDelegate>)cache andCachePolicy:(CCMyASICachePolicy)policy
{
	CCMyASIHTTPRequest *request = [[[self alloc] initWithURL:newURL] autorelease];
...
- (void)buildPostBody
{
	...
	if ([self postBodyFilePath]) {
		...
		if ([self shouldCompressRequestBody]) {
			if (![self compressedPostBodyFilePath]) {
				...
				NSError *err = nil;
				if (![CCMyASIDataCompressor compressDataFromFile:[self postBodyFilePath] toFile:[self compressedPostBodyFilePath] error:&err]) {
		...
		if (err) {
			[self failWithError:[NSError errorWithDomain:CCMyNetworkRequestErrorDomain code:ASIFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to get attributes for file at path '%@'",path],NSLocalizedDescriptionKey,error,NSUnderlyingErrorKey,nil]]];
			return;
		}
	...
	// Otherwise, we have an in-memory request body
		...
		if ([self shouldCompressRequestBody]) {
			NSError *err = nil;
			NSData *compressedBody = [CCMyASIDataCompressor compressData:[self postBody] error:&err];
			if (err) {
				[self failWithError:err];
				return;
			}
...
- (void)cancelOnRequestThread
{
	...
	[self failWithError:CCMyASIRequestCancelledError];
...
{	
	if ([self isResponseCompressed] && [self shouldWaitToInflateCompressedResponses]) {
		return [CCMyASIDataDecompressor uncompressData:[self rawResponseData] error:NULL];
	} else {
...
- (void)startSynchronous
{
	...
	[self setRunLoopMode:CCMyASIHTTPRequestRunLoopMode];
...
#endif
	[CCMysharedQueue addOperation:self];
}
...
- (void)main
{
	@try {
		...
		if ([CCMyASIHTTPRequest isMultitaskingSupported] && [self shouldContinueWhenAppEntersBackground]) {
		...
		if (![self url]) {
			[self failWithError:CCMyASIUnableToCreateRequestError];
			return;		
		}
		...
		NSError *underlyingError = [NSError errorWithDomain:CCMyNetworkRequestErrorDomain code:ASIUnhandledExceptionError userInfo:[exception userInfo]];
		[self failWithError:[NSError errorWithDomain:CCMyNetworkRequestErrorDomain code:ASIUnhandledExceptionError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[exception name],NSLocalizedDescriptionKey,[exception reason],NSLocalizedFailureReasonErrorKey,underlyingError,NSUnderlyingErrorKey,nil]]];
...
- (void)buildRequestHeaders
{
	...
	if (![[self requestHeaders] objectForKey:@"User-Agent"]) {
		...
		if (!tempUserAgentString) {
			tempUserAgentString = [CCMyASIHTTPRequest defaultUserAgentString];
		}
		if (tempUserAgentString) {
...
- (void)startRequest
{
	...
	if ([self shouldStreamPostDataFromDisk] && [self postBodyFilePath] && [fileManager fileExistsAtPath:[self postBodyFilePath]]) {
		...
		if ([self compressedPostBodyFilePath] && [fileManager fileExistsAtPath:[self compressedPostBodyFilePath]]) {
			[self setPostBodyReadStream:[CCMyASIInputStream inputStreamWithFileAtPath:[self compressedPostBodyFilePath] request:self]];
		} else {
			[self setPostBodyReadStream:[CCMyASIInputStream inputStreamWithFileAtPath:[self postBodyFilePath] request:self]];
		}
		...
		if ([self postBody] && [[self postBody] length] > 0) {
			if ([self shouldCompressRequestBody] && [self compressedPostBody]) {
				[self setPostBodyReadStream:[CCMyASIInputStream inputStreamWithData:[self compressedPostBody] request:self]];
			} else if ([self postBody]) {
				[self setPostBodyReadStream:[CCMyASIInputStream inputStreamWithData:[self postBody] request:self]];
			}
			[self setReadStream:[NSMakeCollectable(CFReadStreamCreateForStreamedHTTPRequest(kCFAllocatorDefault, request,(CFReadStreamRef)[self postBodyReadStream])) autorelease]];
	...
	if (![self readStream]) {
		[self failWithError:[NSError errorWithDomain:CCMyNetworkRequestErrorDomain code:ASIInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to create read stream",NSLocalizedDescriptionKey,nil]]];
        return;
    }
	...
	[CCMyASIHTTPRequest expirePersistentConnections];

	[CCMyconnectionsLock lock];
	...
	if ([self shouldAttemptPersistentConnection]) {
		...
		if ([self connectionInfo]) {
			...
			// Check if we should have expired this connection
				...
				#endif
				[CCMypersistentConnectionsPool removeObject:[self connectionInfo]];
				[self setConnectionInfo:nil];
		...
		if (![self connectionInfo] && [[self url] host] && [[self url] scheme]) { // We must have a proper url with a host and scheme, or this will explode
			...
			for (NSMutableDictionary *existingConnection in CCMypersistentConnectionsPool) {
		...
		if (![self connectionInfo]) {
			...
			CCMynextConnectionNumberToCreate++;
			[[self connectionInfo] setObject:[NSNumber numberWithInt:(int)CCMynextConnectionNumberToCreate] forKey:@"id"];
			...
			[CCMypersistentConnectionsPool addObject:[self connectionInfo]];
		}
		
		// If we are retrying this request, it will already have a requestID
		if (![self requestID]) {
			CCMynextRequestID++;
			[self setRequestID:[NSNumber numberWithUnsignedInt:CCMynextRequestID]];
		}
	...
	[CCMyconnectionsLock unlock];

	// Schedule the stream
	if (![self readStreamIsScheduled] && (!CCMythrottleWakeUpTime || [CCMythrottleWakeUpTime timeIntervalSinceDate:[NSDate date]] < 0)) {
   ...
   // Start the HTTP connection
    ...
    if (CFReadStreamSetClient((CFReadStreamRef)[self readStream], CCMykNetworkEvents, CCMyReadStreamClientCallBack, &ctxt)) {
	...
	if (!streamSuccessfullyOpened) {
		...
		[self failWithError:[NSError errorWithDomain:CCMyNetworkRequestErrorDomain code:ASIInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to start HTTP connection",NSLocalizedDescriptionKey,nil]]];
		return;	
	}
	...
	if (![self mainRequest]) {
		if ([self shouldResetUploadProgress]) {
			...
			[CCMyASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:0 ofTotal:1];
		}
		if ([self shouldResetDownloadProgress] && ![self partialDownloadSize]) {
			[CCMyASIHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:0 ofTotal:1];
		}
	}	
...
- (void)checkRequestStatus
{
	...
	if ([self shouldTimeOut]) {			
		...
		[self failWithError:CCMyASIRequestTimedOutError];
		[self cancelLoad];
		[self setComplete:YES];
		[[self cancelledLock] unlock];
		return;
	...
	if ([self readStream]) {
		...
		if ([self postLength]) {
			...
			if (totalBytesSent > lastBytesSent) {
				...
				[self setLastActivityTime:[NSDate date]];
				[CCMyASIHTTPRequest incrementBandwidthUsedInLastSecond:(unsigned long)(totalBytesSent-lastBytesSent)];
...
- (CCMyASIHTTPRequest *)HEADRequest
{
	CCMyASIHTTPRequest *headRequest = [[self class] requestWithURL:[self url]];
...
- (void)setUploadProgressDelegate:(id)newDelegate
{
	...
	double max = 1.0;
	[CCMyASIHTTPRequest performSelector:@selector(setMaxValue:) onTarget:&uploadProgressDelegate withObject:nil amount:&max callerToRetain:nil];
	#endif
	[[self cancelledLock] unlock];
}
...
- (void)setDownloadProgressDelegate:(id)newDelegate
{
	...
	double max = 1.0;
	[CCMyASIHTTPRequest performSelector:@selector(setMaxValue:) onTarget:&downloadProgressDelegate withObject:nil amount:&max callerToRetain:nil];
	#endif
	[[self cancelledLock] unlock];
}
...
- (void)updateDownloadProgress
{
	...
	[CCMyASIHTTPRequest performSelector:@selector(request:didReceiveBytes:) onTarget:&queue withObject:self amount:&value callerToRetain:self];
	[CCMyASIHTTPRequest performSelector:@selector(request:didReceiveBytes:) onTarget:&downloadProgressDelegate withObject:self amount:&value callerToRetain:self];

	[CCMyASIHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:[self totalBytesRead]+[self partialDownloadSize] ofTotal:[self contentLength]+[self partialDownloadSize]];
...
- (void)updateUploadProgress
{
	...
	[CCMyASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&queue withObject:self amount:&value callerToRetain:self];
	[CCMyASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&uploadProgressDelegate withObject:self amount:&value callerToRetain:self];
	[CCMyASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:[self totalBytesSent]-[self uploadBufferSize] ofTotal:[self postLength]-[self uploadBufferSize]];
...
{
	[CCMyASIHTTPRequest performSelector:@selector(request:incrementDownloadSizeBy:) onTarget:&queue withObject:self amount:&length callerToRetain:self];
	[CCMyASIHTTPRequest performSelector:@selector(request:incrementDownloadSizeBy:) onTarget:&downloadProgressDelegate withObject:self amount:&length callerToRetain:self];
...
{
	[CCMyASIHTTPRequest performSelector:@selector(request:incrementUploadSizeBy:) onTarget:&queue withObject:self amount:&length callerToRetain:self];
	[CCMyASIHTTPRequest performSelector:@selector(request:incrementUploadSizeBy:) onTarget:&uploadProgressDelegate withObject:self amount:&length callerToRetain:self];
...
-(void)removeUploadProgressSoFar
{
	...
	[CCMyASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&queue withObject:self amount:&progressToRemove callerToRetain:self];
	[CCMyASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&uploadProgressDelegate withObject:self amount:&progressToRemove callerToRetain:self];
	[CCMyASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:0 ofTotal:[self postLength]];
...
+ (void)performSelector:(SEL)selector onTarget:(id *)target withObject:(id)object amount:(void *)amount callerToRetain:(id)callerToRetain
{
	if ([*target respondsToSelector:selector]) {
        ...
        NSMethodSignature *cbSignature = [CCMyASIHTTPRequest methodSignatureForSelector:callback];
...
+ (void)updateProgressIndicator:(id *)indicator withProgress:(unsigned long long)progress ofTotal:(unsigned long long)total
{
	...
	[CCMyprogressLock lock];
	[CCMyASIHTTPRequest performSelector:selector onTarget:indicator withObject:nil amount:&progressAmount callerToRetain:nil];
	[CCMyprogressLock unlock];
}
...
- (void)failWithError:(NSError *)theError
{
	...
	CCMyASIHTTPRequest *failedRequest = self;
...
- (void)readResponseHeaders
{
	...
	if ([self useCookiePersistence]) {
		...
		for (cookie in newCookies) {
			[CCMyASIHTTPRequest addSessionCookie:cookie];
		}
	}
	...
	if (![self willRedirect]) {
		...
		CCMyASIHTTPRequest *theRequest = self;
		if ([self mainRequest]) {
...
- (void)saveProxyCredentialsToKeychain:(NSDictionary *)newCredentials
{
	...
	if (authenticationCredentials) {
		[CCMyASIHTTPRequest saveCredentials:authenticationCredentials forProxy:[self proxyHost] port:[self proxyPort] realm:[self proxyAuthenticationRealm]];
	}	
}
...
- (void)saveCredentialsToKeychain:(NSDictionary *)newCredentials
{
	...
	if (authenticationCredentials) {
		[CCMyASIHTTPRequest saveCredentials:authenticationCredentials forHost:[[self url] host] port:[[[self url] port] intValue] protocol:[[self url] scheme] realm:[self authenticationRealm]];
	}	
}
...
- (NSMutableDictionary *)findProxyCredentials
{
	...
	CCMyASIHTTPRequest *theRequest = [self mainRequest];
	...
	if ((!user || !pass)) {
		NSURLCredential *authenticationCredentials = [CCMyASIHTTPRequest savedCredentialsForProxy:[self proxyHost] port:[self proxyPort] protocol:[[self url] scheme] realm:[self proxyAuthenticationRealm]];
		if (authenticationCredentials) {
...
- (NSMutableDictionary *)findCredentials
{
	...
	if ((!user || !pass) && useKeychainPersistence) {
		NSURLCredential *authenticationCredentials = [CCMyASIHTTPRequest savedCredentialsForHost:[[self url] host] port:[[[self url] port] intValue] protocol:[[self url] scheme] realm:[self authenticationRealm]];
		if (authenticationCredentials) {
			user = [authenticationCredentials user];
			pass = [authenticationCredentials password];
			#if DEBUG_HTTP_AUTHENTICATION
...
- (void)failAuthentication
{
	[self failWithError:CCMyASIAuthenticationError];
}
...
- (BOOL)showProxyAuthenticationDialog
{
	...
	if ([self shouldPresentProxyAuthenticationDialog]) {
		[CCMyASIAuthenticationDialog performSelectorOnMainThread:@selector(presentAuthenticationDialogForRequest:) withObject:self waitUntilDone:[NSThread isMainThread]];
		return YES;
	}
	return NO;
	#else
...
- (void)attemptToApplyProxyCredentialsAndResume
{
	...
	if (!proxyAuthentication) {
		[self cancelLoad];
		[self failWithError:[NSError errorWithDomain:CCMyNetworkRequestErrorDomain code:ASIInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to get authentication object from response headers",NSLocalizedDescriptionKey,nil]]];
		return;
	}
	...
	if (!CFHTTPAuthenticationIsValid(proxyAuthentication, &err)) {
		...
		if (err.domain == kCFStreamErrorDomainHTTP && (err.error == kCFStreamErrorHTTPAuthenticationBadUserName || err.error == kCFStreamErrorHTTPAuthenticationBadPassword)) {
			...
			[CCMydelegateAuthenticationLock lock];
			...
			if ([self error] || [self isCancelled]) {
				[CCMydelegateAuthenticationLock unlock];
				return;
			}
	...
	if (proxyCredentials) {
		...
		// Something went wrong, we'll have to give up
			...
			[self failWithError:[NSError errorWithDomain:CCMyNetworkRequestErrorDomain code:ASIInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to apply proxy credentials to request",NSLocalizedDescriptionKey,nil]]];
		}
...
- (void)attemptToApplyCredentialsAndResume
{
	...
	if (requestCredentials) {
		...
		if ((([self authenticationScheme] != (NSString *)kCFHTTPAuthenticationSchemeNTLM) || [self authenticationRetryCount] < 2) && [self applyCredentials:requestCredentials]) {
			...
			[self failWithError:[NSError errorWithDomain:CCMyNetworkRequestErrorDomain code:ASIInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to apply credentials to request",NSLocalizedDescriptionKey,nil]]];
		}
...
{
	[self addRequestHeader:@"Authorization" value:[NSString stringWithFormat:@"Basic %@",[CCMyASIHTTPRequest base64forData:[[NSString stringWithFormat:@"%@:%@",theUsername,thePassword] dataUsingEncoding:NSUTF8StringEncoding]]]];
...
- (void)handleBytesAvailable
{
	...
	if ([[self class] isBandwidthThrottled]) {
		[CCMybandwidthThrottlingLock lock];
		if (CCMymaxBandwidthPerSecond > 0) {
			long long maxiumumSize  = (long long)CCMymaxBandwidthPerSecond-(long long)CCMybandwidthUsedInLastSecond;
			if (maxiumumSize < 0) {
		...
		[CCMybandwidthThrottlingLock unlock];
	}
	...
	// If zero bytes were read, wait for the EOF to come.
		...
		if ([self isResponseCompressed] && ![self shouldWaitToInflateCompressedResponses]) {
			if (![self dataDecompressor]) {
				[self setDataDecompressor:[CCMyASIDataDecompressor decompressor]];
			}
			NSError *err = nil;
		...
		[CCMyASIHTTPRequest incrementBandwidthUsedInLastSecond:(NSUInteger)bytesRead];
	...
	if ([self temporaryFileDownloadPath]) {
		...
		if ([self shouldRedirect] && [self needsRedirect] && [self allowResumeForFileDownloads]) {
			...
			if ([self shouldWaitToInflateCompressedResponses]) {
				[CCMyASIDataDecompressor uncompressDataFromFile:[self temporaryFileDownloadPath] toFile:[self downloadDestinationPath] error:&fileError];
...

{
	...
	if (![self error]) { // We may already have handled this error
		...
		[self cancelLoad];
		[self failWithError:[NSError errorWithDomain:CCMyNetworkRequestErrorDomain code:ASIConnectionFailureErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:reason,NSLocalizedDescriptionKey,underlyingError,NSUnderlyingErrorKey,nil]]];
	} else {
		[self cancelLoad];
	}
	[self checkRequestStatus];
...
- (void)scheduleReadStream
{
	if ([self readStream] && ![self readStreamIsScheduled]) {
		...
		[CCMyconnectionsLock lock];
		CCMyrunningRequestCount++;
		if (CCMyshouldUpdateNetworkActivityIndicator) {
		...
		[self setLastActivityTime:[NSDate date]];
		CFStreamClientContext ctxt = {0, self, NULL, NULL, NULL};
		CFReadStreamSetClient((CFReadStreamRef)[self readStream], CCMykNetworkEvents, CCMyReadStreamClientCallBack, &ctxt);
...
- (void)unscheduleReadStream
{
	if ([self readStream] && [self readStreamIsScheduled]) {
		...
		[CCMyconnectionsLock lock];
		CCMyrunningRequestCount--;
		if (CCMyshouldUpdateNetworkActivityIndicator && CCMyrunningRequestCount == 0) {
...
+ (BOOL)removeFileAtPath:(NSString *)path error:(NSError **)err
{
	...
	if ([fileManager fileExistsAtPath:path]) {
		...
		if (removeError) {
			if (err) {
				*err = [NSError errorWithDomain:CCMyNetworkRequestErrorDomain code:ASIFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to delete file at path '%@'",path],NSLocalizedDescriptionKey,removeError,NSUnderlyingErrorKey,nil]];
			}
			return NO;
		}
	}
...
- (BOOL)configureProxies
{
	...
	if (![self isPACFileRequest] && (![self proxyHost] && ![self proxyPort])) {
		...
		if (!proxies) {
			[self setReadStream:nil];
			[self failWithError:[NSError errorWithDomain:CCMyNetworkRequestErrorDomain code:ASIInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to obtain information on proxy servers needed for request",NSLocalizedDescriptionKey,nil]]];
			return NO;
		}
...
- (void)fetchPACFile
{
	...
	CCMyASIHTTPRequest *PACRequest = [CCMyASIHTTPRequest requestWithURL:[self PACurl]];
	...
	[CCMyconnectionsLock lock];
	[CCMysharedQueue setMaxConcurrentOperationCount:[CCMysharedQueue maxConcurrentOperationCount]+1];
	[CCMyconnectionsLock unlock];
}
...
+ (void)expirePersistentConnections
{
	[CCMyconnectionsLock lock];
	NSUInteger i;
	for (i=0; i<[CCMypersistentConnectionsPool count]; i++) {
		NSDictionary *existingConnection = [CCMypersistentConnectionsPool objectAtIndex:i];
		if (![existingConnection objectForKey:@"request"] && [[existingConnection objectForKey:@"expires"] timeIntervalSinceNow] <= 0) {
			...
			[CCMypersistentConnectionsPool removeObject:existingConnection];
			i--;
		}
	}	
	[CCMyconnectionsLock unlock];
...
- (id)copyWithZone:(NSZone *)zone
{
	...
	CCMyASIHTTPRequest *newRequest = [[[self class] alloc] initWithURL:[self url]];
...
{
	return CCMydefaultTimeOutSeconds;
}

+ (void)setDefaultTimeOutSeconds:(NSTimeInterval)newTimeOutSeconds
{
	CCMydefaultTimeOutSeconds = newTimeOutSeconds;
}
...
{
	[CCMysessionCredentialsLock lock];
	if (!CCMysessionProxyCredentialsStore) {
		CCMysessionProxyCredentialsStore = [[NSMutableArray alloc] init];
	}
	[CCMysessionCredentialsLock unlock];
	return CCMysessionProxyCredentialsStore;
}

+ (NSMutableArray *)sessionCredentialsStore
{
	[CCMysessionCredentialsLock lock];
	if (!CCMysessionCredentialsStore) {
		CCMysessionCredentialsStore = [[NSMutableArray alloc] init];
	}
	[CCMysessionCredentialsLock unlock];
	return CCMysessionCredentialsStore;
}
...
{
	[CCMysessionCookiesLock lock];
	if (!CCMysessionCookies) {
		[CCMyASIHTTPRequest setSessionCookies:[NSMutableArray array]];
	}
	NSMutableArray *cookies = [[CCMysessionCookies retain] autorelease];
	[CCMysessionCookiesLock unlock];
	return cookies;
}
...
+ (void)setSessionCookies:(NSMutableArray *)newSessionCookies
{
	...
	for (NSHTTPCookie *cookie in CCMysessionCookies) {
		[[NSHTTPCookieStorage sharedHTTPCookieStorage] deleteCookie:cookie];
	}
	[CCMysessionCookies release];
	CCMysessionCookies = [newSessionCookies retain];
	[CCMysessionCookiesLock unlock];
}
...
+ (void)addSessionCookie:(NSHTTPCookie *)newCookie
{
	...
	NSHTTPCookie *cookie;
	NSUInteger i;
	NSUInteger max = [[CCMyASIHTTPRequest sessionCookies] count];
	for (i=0; i<max; i++) {
		cookie = [[CCMyASIHTTPRequest sessionCookies] objectAtIndex:i];
		if ([[cookie domain] isEqualToString:[newCookie domain]] && [[cookie path] isEqualToString:[newCookie path]] && [[cookie name] isEqualToString:[newCookie name]]) {
			[[CCMyASIHTTPRequest sessionCookies] removeObjectAtIndex:i];
			break;
		}
	}
	[[CCMyASIHTTPRequest sessionCookies] addObject:newCookie];
	[CCMysessionCookiesLock unlock];
}
...
+ (NSString *)defaultUserAgentString
{
	@synchronized (self) {
		...
		return [[CCMydefaultUserAgent retain] autorelease];
	}
	return nil;
}
...
{
	@synchronized (self) {
		if (CCMydefaultUserAgent == agent) {
			return;
		}
		[CCMydefaultUserAgent release];
		CCMydefaultUserAgent = [agent copy];
	}
}
...
- (void)performThrottling
{
	...
	[CCMyASIHTTPRequest measureBandwidthUsage];
	if ([CCMyASIHTTPRequest isBandwidthThrottled]) {
		[CCMybandwidthThrottlingLock lock];
		// Handle throttling
		if (CCMythrottleWakeUpTime) {
			if ([CCMythrottleWakeUpTime timeIntervalSinceDate:[NSDate date]] > 0) {
...
+ (BOOL)isBandwidthThrottled
{
	...
	BOOL throttle = CCMyisBandwidthThrottled || (!CCMyshouldThrottleBandwidthForWWANOnly && (CCMymaxBandwidthPerSecond > 0));
	[CCMybandwidthThrottlingLock unlock];
	return throttle;
#else
...
{
	[CCMybandwidthThrottlingLock lock];
	unsigned long amount = CCMymaxBandwidthPerSecond;
	[CCMybandwidthThrottlingLock unlock];
	return amount;
}
...
{
	[CCMybandwidthThrottlingLock lock];
	CCMymaxBandwidthPerSecond = bytes;
	[CCMybandwidthThrottlingLock unlock];
}
...
{
	[CCMybandwidthThrottlingLock lock];
	CCMybandwidthUsedInLastSecond += bytes;
	[CCMybandwidthThrottlingLock unlock];
}

+ (void)recordBandwidthUsage
{
	if (CCMybandwidthUsedInLastSecond == 0) {
		[CCMybandwidthUsageTracker removeAllObjects];
	} else {
		NSTimeInterval interval = [CCMybandwidthMeasurementDate timeIntervalSinceNow];
		while ((interval < 0 || [CCMybandwidthUsageTracker count] > 5) && [CCMybandwidthUsageTracker count] > 0) {
			[CCMybandwidthUsageTracker removeObjectAtIndex:0];
			interval++;
		}
	}
	#if DEBUG_THROTTLING
	...
	#endif
	[CCMybandwidthUsageTracker addObject:[NSNumber numberWithUnsignedLong:CCMybandwidthUsedInLastSecond]];
	[CCMybandwidthMeasurementDate release];
	CCMybandwidthMeasurementDate = [[NSDate dateWithTimeIntervalSinceNow:1] retain];
	CCMybandwidthUsedInLastSecond = 0;
	
	NSUInteger measurements = [CCMybandwidthUsageTracker count];
	unsigned long totalBytes = 0;
	for (NSNumber *bytes in CCMybandwidthUsageTracker) {
		totalBytes += [bytes unsignedLongValue];
	}
	if (measurements > 0)
		CCMyaverageBandwidthUsedPerSecond = totalBytes/measurements;
}

+ (unsigned long)averageBandwidthUsedPerSecond
{
	[CCMybandwidthThrottlingLock lock];
	unsigned long amount = 	CCMyaverageBandwidthUsedPerSecond;
	[CCMybandwidthThrottlingLock unlock];
	return amount;
}
...
+ (void)measureBandwidthUsage
{
	...
	if (!CCMybandwidthMeasurementDate || [CCMybandwidthMeasurementDate timeIntervalSinceNow] < -0) {
		[CCMyASIHTTPRequest recordBandwidthUsage];
	}
	
	// Are we performing bandwidth throttling?
	if (
	#if TARGET_OS_IPHONE
	CCMyisBandwidthThrottled || (!CCMyshouldThrottleBandwidthForWWANOnly && (CCMymaxBandwidthPerSecond))
	#else
	maxBandwidthPerSecond
	#endif
	) {
		// How much data can we still send or receive this second?
		long long bytesRemaining = (long long)CCMymaxBandwidthPerSecond - (long long)CCMybandwidthUsedInLastSecond;
		...
		if (bytesRemaining < 0) {
			...
			double extraSleepyTime = (-bytesRemaining/(CCMymaxBandwidthPerSecond*1.0));
			[CCMythrottleWakeUpTime release];
			CCMythrottleWakeUpTime = [[NSDate alloc] initWithTimeInterval:extraSleepyTime sinceDate:CCMybandwidthMeasurementDate];
		}
	}
	[CCMybandwidthThrottlingLock unlock];
}
...
+ (unsigned long)maxUploadReadLength
{
	...
	long long toRead = CCMymaxBandwidthPerSecond/4;
	if (CCMymaxBandwidthPerSecond > 0 && (CCMybandwidthUsedInLastSecond + toRead > CCMymaxBandwidthPerSecond)) {
		toRead = (long long)CCMymaxBandwidthPerSecond-(long long)CCMybandwidthUsedInLastSecond;
		if (toRead < 0) {
			toRead = 0;
		}
	}
	
	if (toRead == 0 || !CCMybandwidthMeasurementDate || [CCMybandwidthMeasurementDate timeIntervalSinceNow] < -0) {
		[CCMythrottleWakeUpTime release];
		CCMythrottleWakeUpTime = [CCMybandwidthMeasurementDate retain];
	}
	[CCMybandwidthThrottlingLock unlock];
...
{
	if (throttle) {
		[CCMyASIHTTPRequest throttleBandwidthForWWANUsingLimit:CCMyASIWWANBandwidthThrottleAmount];
	} else {
		[CCMyASIHTTPRequest unsubscribeFromNetworkReachabilityNotifications];
		[CCMyASIHTTPRequest setMaxBandwidthPerSecond:0];
		[CCMybandwidthThrottlingLock lock];
		CCMyisBandwidthThrottled = NO;
		CCMyshouldThrottleBandwidthForWWANOnly = NO;
		[CCMybandwidthThrottlingLock unlock];
	}
}
...
{	
	[CCMybandwidthThrottlingLock lock];
	CCMyshouldThrottleBandwidthForWWANOnly = YES;
	CCMymaxBandwidthPerSecond = limit;
	[CCMyASIHTTPRequest registerForNetworkReachabilityNotifications];
	[CCMybandwidthThrottlingLock unlock];
	[CCMyASIHTTPRequest reachabilityChanged:nil];
}
...
+ (void)registerForNetworkReachabilityNotifications
{
	[[CCMyReachability reachabilityForInternetConnection] startNotifier];
	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(reachabilityChanged:) name:CCMykReachabilityChangedNotification object:nil];
}


+ (void)unsubscribeFromNetworkReachabilityNotifications
{
	[[NSNotificationCenter defaultCenter] removeObserver:self name:CCMykReachabilityChangedNotification object:nil];
}

+ (BOOL)isNetworkReachableViaWWAN
{
	return ([[CCMyReachability reachabilityForInternetConnection] currentReachabilityStatus] == ReachableViaWWAN);
}

+ (void)reachabilityChanged:(NSNotification *)note
{
	[CCMybandwidthThrottlingLock lock];
	CCMyisBandwidthThrottled = [CCMyASIHTTPRequest isNetworkReachableViaWWAN];
	[CCMybandwidthThrottlingLock unlock];
}
#endif
...
{
    return [[CCMysharedQueue retain] autorelease];
}

#pragma mark cache

+ (void)setDefaultCache:(id <CCMyASICacheDelegate>)cache
{
	@synchronized (self) {
		[cache retain];
		[CCMydefaultCache release];
		CCMydefaultCache = cache;
	}
}

+ (id <CCMyASICacheDelegate>)defaultCache
{
    @synchronized(self) {
        return [[CCMydefaultCache retain] autorelease];
    }
	return nil;
}
...
{
	[CCMyconnectionsLock lock];
	BOOL inUse = (CCMyrunningRequestCount > 0);
	[CCMyconnectionsLock unlock];
	return inUse;
}
...
{
	[CCMyconnectionsLock lock];
	CCMyshouldUpdateNetworkActivityIndicator = shouldUpdate;
	[CCMyconnectionsLock unlock];
}
...
+ (NSThread *)threadForRequest:(CCMyASIHTTPRequest *)request
{
	if (CCMynetworkThread == nil) {
		@synchronized(self) {
			if (CCMynetworkThread == nil) {
				CCMynetworkThread = [[NSThread alloc] initWithTarget:self selector:@selector(runRequests) object:nil];
				[CCMynetworkThread start];
			}
		}
	}
	return CCMynetworkThread;
}
...
+ (NSDate *)expiryDateForRequest:(CCMyASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
...
#pragma clang diagnostic pop
		...
		if (expires) {
			return [CCMyASIHTTPRequest dateFromRFC1123String:expires];
		}
	}
	return nil;
}
