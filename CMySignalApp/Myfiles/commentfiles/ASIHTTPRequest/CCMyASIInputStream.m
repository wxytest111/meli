...
static NSLock *CCMyreadLock = nil;

@implementation CCMyASIInputStream

+ (void)initialize
{
	if (self == [CCMyASIInputStream class]) {
		CCMyreadLock = [[NSLock alloc] init];
	}
}

+ (id)inputStreamWithFileAtPath:(NSString *)path request:(CCMyASIHTTPRequest *)theRequest
{
	CCMyASIInputStream *theStream = [[[self alloc] init] autorelease];
	[theStream setRequest:theRequest];
	[theStream setStream:[NSInputStream inputStreamWithFileAtPath:path]];
	return theStream;
}

+ (id)inputStreamWithData:(NSData *)data request:(CCMyASIHTTPRequest *)theRequest
{
	CCMyASIInputStream *theStream = [[[self alloc] init] autorelease];
	[theStream setRequest:theRequest];
...
- (NSInteger)read:(uint8_t *)buffer maxLength:(NSUInteger)len
{
	...
	if ([CCMyASIHTTPRequest isBandwidthThrottled]) {
		toRead = [CCMyASIHTTPRequest maxUploadReadLength];
		if (toRead > len) {
			toRead = len;
		} else if (toRead == 0) {
			toRead = 1;
	...
	if (rv > 0)
		[CCMyASIHTTPRequest incrementBandwidthUsedInLastSecond:(NSUInteger)rv];
	return rv;
}
