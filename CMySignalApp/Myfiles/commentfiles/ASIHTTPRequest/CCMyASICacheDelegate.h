...
typedef enum _CCMyASICachePolicy {

	// The default cache policy. When you set a request to use this, it will use the cache's defaultCachePolicy
	// CCMyASIDownloadCache's default cache policy is 'ASIAskServerIfModifiedWhenStaleCachePolicy'
...
typedef enum _CCMyASICacheStoragePolicy {
	ASICacheForSessionDurationCacheStoragePolicy = 0,
	ASICachePermanentlyCacheStoragePolicy = 1
} CCMyASICacheStoragePolicy;


@protocol CCMyASICacheDelegate <NSObject>
...
- (void)clearCachedResponsesForStoragePolicy:(CCMyASICacheStoragePolicy)cachePolicy;
