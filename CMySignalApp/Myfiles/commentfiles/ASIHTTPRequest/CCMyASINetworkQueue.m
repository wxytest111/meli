...
- (void)dealloc
{
	...
	for (CCMyASIHTTPRequest *request in [self operations]) {
...
- (void)resetProgressDelegate:(id *)progressDelegate
{
	...
	if ([*progressDelegate respondsToSelector:selector]) {
		double max = 1.0;
		[CCMyASIHTTPRequest performSelector:selector onTarget:progressDelegate withObject:nil amount:&max callerToRetain:nil];
	}
	selector = @selector(setDoubleValue:);
	if ([*progressDelegate respondsToSelector:selector]) {
		double value = 0.0;
		[CCMyASIHTTPRequest performSelector:selector onTarget:progressDelegate withObject:nil amount:&value callerToRetain:nil];
	}
#else
...
{
	if ([operation isKindOfClass:[CCMyASIHTTPRequest class]]) {
		
		CCMyASIHTTPRequest *request = (CCMyASIHTTPRequest *)operation;
...
- (void)addOperation:(NSOperation *)operation
{
	...
	if ([self showAccurateProgress]) {
		...
		if ([[request requestMethod] isEqualToString:@"GET"]) {
			if ([self isSuspended]) {
				CCMyASIHTTPRequest *HEADRequest = [request HEADRequest];
...
- (void)request:(CCMyASIHTTPRequest *)request didReceiveBytes:(long long)bytes
{
	[self setBytesDownloadedSoFar:[self bytesDownloadedSoFar]+(unsigned long long)bytes];
	if ([self downloadProgressDelegate]) {
		[CCMyASIHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:[self bytesDownloadedSoFar] ofTotal:[self totalBytesToDownload]];
	}
}

- (void)request:(CCMyASIHTTPRequest *)request didSendBytes:(long long)bytes
{
	[self setBytesUploadedSoFar:[self bytesUploadedSoFar]+(unsigned long long)bytes];
	if ([self uploadProgressDelegate]) {
		[CCMyASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:[self bytesUploadedSoFar] ofTotal:[self totalBytesToUpload]];
	}
}

- (void)request:(CCMyASIHTTPRequest *)request incrementDownloadSizeBy:(long long)newLength
{
	[self setTotalBytesToDownload:[self totalBytesToDownload]+(unsigned long long)newLength];
}

- (void)request:(CCMyASIHTTPRequest *)request incrementUploadSizeBy:(long long)newLength
{
...
{
	CCMyASINetworkQueue *newQueue = [[[self class] alloc] init];
